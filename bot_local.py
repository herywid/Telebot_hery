...
def send_about(message):
    ...
    with open('template_text/about.txt', mode='r', encoding='utf-8') as f:
        ...
        about = temp.substitute(
            STUDENT_NAME = "HeryWidjayanto",
            BATCH_ACADEMY = "QoppaNight",
            GITHUB_REPO_LINK = "https://github.com/herywid/Telebot_hery.git"
        )
...
unique_campaign = np.array(df['campaign_id'].unique())
...
def send_summary(message):
    ...
    if selected_campaign_id in unique_campaign:
        ...
        df_campaign = df.loc[df["campaign_id"].isin([selected_campaign_id])].reset_index(drop=True)
